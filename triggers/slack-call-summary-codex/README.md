...
Headlessly runs [Codex](https://developers.openai.com/codex/cli/) when Tuple Capture completes: it summarizes the triggering recording, writes a title and summary back onto the call (Tuple's Call History), and DMs the summary to Slack.
...
It's a one-shot over the finished call — no `tuple connect`, nothing live to follow. Codex uses the triggering call and recording IDs, reads that recording's stored transcript with the `tuple` CLI, and does the work.
...
3. Codex uses the trigger's call and recording IDs, reads transcript and event records with `tuple capture show --recording "$TUPLE_TRIGGER_RECORDING_ID" --exclude content --format json`, writes the title and summary with `tuple call edit`, and sends the Slack message.
